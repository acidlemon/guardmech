...
func (a *AuthMux) RegisterRoute(r *mux.Router) {
	r.HandleFunc("/auth/start", a.StartAuth)
	r.HandleFunc("/auth/callback", a.CallbackAuth)
	r.HandleFunc("/auth/request", a.AuthRequest)
	r.HandleFunc("/auth/sign_in", a.SignIn)
	r.HandleFunc("/auth/unauthorized", a.Unauthorized)
}
...
func (a *AuthMux) SignIn(w http.ResponseWriter, req *http.Request) {
	...
	prefix := os.Getenv("GUARDMECH_MOUNT_PATH")
	...
	if err == nil {
		...
		if err != nil {
			http.Redirect(w, req, fmt.Sprintf("%s/auth/start?p=%s", prefix, path), http.StatusFound)
			return
		}
...
<main>
  ...
  <form method="get" action="` +
		os.Getenv("GUARDMECH_MOUNT_PATH") + // TODO とても行儀が悪いので直す
		`/auth/start">
